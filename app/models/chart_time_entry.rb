class ChartTimeEntry < ActiveRecord::Base
  ...
  def self.get_timeline(raw_group, raw_conditions, range)
    ...
    unless range
      row = select("month, week, day").joins(joins).where(["day > 0 AND chart_time_entries.project_id in (?)", conditions['chart_time_entries.project_id']]).order("1 asc, 2 asc, 3 asc").first
    ...
    rows = select(select).joins(joins).where(conditions).group(grouping).order("1 asc, 6 asc")
  ...
  def self.get_aggregation_for_issue(raw_conditions, range)
    ...
    rows = select(select).joins(joins).where(conditions).group(group).order("1 desc, 2 asc")
  ...
  def self.get_aggregation(raw_group, raw_conditions)
    ...
    rows = select(select).joins(joins).where(conditions).group(group).order("1 desc, 3 asc")
